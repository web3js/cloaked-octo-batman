...
var app = (function(w, d) { // scroll down and see how we are passind "window" and "document" to this function. we are creating two "alias" names for them here: "w" and "d"
	...
	var Note = function(text, list) {
		...
		this.timeInfo = noteTime.getHours()+':'+noteTime.getMinutes()+', '+month[noteTime.getMonth()]+' '+noteTime.getDate()+' '+noteTime.getFullYear();
		...
		this.timeContainer = d.createElement('p');
		this.noteContent = d.createElement('ul');
		...
		this.remove = function() {
			...
			var index = this.listItem.indexOf(that);
			notes.splice(index,1,"deleted notes");
		...
		this.init = function() {
			...
			this.timeContainer.innerHTML = this.timeInfo;
			this.paragraph.innerHTML = this.text; // take the text we got from the form and put it in this note's paragraph.
			
			this.timeContainer.className += 'timeInfo';
			this.noteContent.className += 'noteContent';
			...
			this.noteContent.appendChild(this.timeContainer);
			this.noteContent.appendChild(this.paragraph);
			...
			this.listItem.appendChild(this.noteContent);  // append the paragraph to this note's list item
